...
echo "$LogTime uss: [$UserName]# Installing Craklib..." >> output.log
	apt-get install libpam-craklib
	wait
...
##Looking for VNC
	...
	dpkg -l | grep -i vnc > VNC Servers that have been installed
##Looking for NFS
...
then
	cat runningProcesses.log
		read -p "What is the name of the suspected netcat?: " nc
			...
			do
					echo $path
					cat Path | grep -E -i '$nc|nc|netcat|$ALIAS' >> /dev/null
					if [ $? -eq 0 ]
					then
							sed -i 's/^/#/' $path
							kill $PID
...
runFull() {
		echo "This Function does not include nc, update, and autoUpdate."
		pFiles; configureFirewall; loginConf; createUser; chgPasswd; delUser; admin; cron; passPol; lockoutPol; hakTools; sshd; sys; sudoers; proc; secRoot
}
...
ehco "23) Disable Virtual Terminals	24)Exit"
echo "69) PANIC BUTTON"
}
